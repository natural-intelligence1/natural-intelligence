...
//
// LEGAL VALUES ARE NOT GUESSED. No per-field Article 6 basis or Article 9
// condition has been solicitor-approved, so every field resolves both to
// the explicit state 'legal_review_required'. The published privacy policy
// (apps/web/app/legal/privacy §4) states candidate bases at POLICY level —
// they are carried below verbatim as candidates for the solicitor to
// confirm per domain, and nothing more.
//
// Retention PERIODS are likewise not approved: every rule carries status
// 'retention_policy_pending'. Deletion eligibility uses the solicitor's
// A/B distinction; nothing implies immediate deletion of health records.
...
export interface V2FieldGovernance {
  ...
  /** UK GDPR Article 6 basis — NOT approved per field; explicit state only. */
  article6Basis: V2LegalBasisState
  /** UK GDPR Article 9 condition — NOT approved per field; explicit state only. */
  article9Condition: V2LegalBasisState
  retentionRuleId: string
  retentionStatus: 'retention_policy_pending'
...
const HEALTH_RECORD_RETENTION = {
  retentionRuleId: 'retention.health_record.v1',
  deletionEligibility: 'retained_subject_to_documented_rule' as const,
  retentionReason:
    'Part of the client health record: may legitimately need retention for professional, legal ' +
    'or claims purposes (matches the approved Screen 1 retention wording). Retention PERIOD is ' +
    'retention_policy_pending — the documented schedule is not yet approved.',
}
...
const DOMAIN_DEFAULTS: Record<V2FieldDomain, DomainDefaults> = {
  identity: {
    ...
    retentionRuleId: 'retention.account_operational.v1',
    deletionEligibility: 'retained_subject_to_documented_rule',
    retentionReason: 'Needed to operate the account while it is active; schedule retention_policy_pending.',
  },
  care_context: {
...
export function resolveV2FieldGovernance(field: V2FieldDef): V2FieldGovernance {
  ...
  return {
    ...
    provenanceBehaviour: field.source === 'practitioner_analysis' || field.practitionerOnly ? 'practitioner_authored'
      ...
      : 'sourced_fact',
    article6Basis: 'legal_review_required',
    article9Condition: 'legal_review_required',
    retentionRuleId: defaults.retentionRuleId,
    retentionStatus: 'retention_policy_pending',
    deletionEligibility: defaults.deletionEligibility,
    retentionReason: defaults.retentionReason,
  }
}
